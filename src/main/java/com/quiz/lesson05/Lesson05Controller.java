...
public class Lesson05Controller {
	...
	// localhost:8080/lesson05/quiz01
	@GetMapping("/quiz01")
	...
	//풀이 중 설명
	//JSP상단에 코어라이브러리 입력 :  <%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
	//view 페이지에서 보이려면 model 이 필요함! : controller와 jsp를 이음!
	// model.addAttribute("key이름은 내맘대로", 리스트 명 작성); >> 동일하게 맞추는게 안 헷갈려~
	// ㄴ JSP에서도 리스트를 사용 가능하게 됨! 
	// JSP에서 마크업 을 한다는게 무슨 말인지?
	// table : tr : 하나의 행 / th : head 칸 / td : 일반칸
	// c:forEach items에서 model의 값을 가져올 때 el문법으로 가져옴 ${} 
	// var : 리스트에 있는 것을 하나하나 가져옴,  items에 있는이름과 다르게 해야함 : 리스트인지 단건인지 헷갈리니까
	//forEach는 오른 에 있는 값을 하나하나씩 왼쪽에 담기는 동작과 동일함 , 하나씩 꺼내지는걸 var(배리어블)에 담아야함
	//순위번호 지정 시 count(1부터 시작)이 필요하므로 varStatus가져오기ㅎ (인덱스는 0부터 시작)
